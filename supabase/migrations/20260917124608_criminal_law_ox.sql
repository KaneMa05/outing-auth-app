-- Authenticated application server only. Student identity is resolved by the API,
-- never taken from a client-supplied actor. No existing application table changes.
create table public.ox_config (id boolean primary key default true check (id), enabled boolean not null default false);
insert into public.ox_config values (true, false);
create table public.ox_collections (id text primary key, entry jsonb not null);
create table public.ox_chapters (id text primary key, collection_id text not null references public.ox_collections(id), entry jsonb not null);
create table public.ox_questions (
  id text primary key, chapter_id text not null references public.ox_chapters(id), entry jsonb not null,
  status text not null default 'draft' check (status in ('draft','published','archived')),
  reviewed boolean not null default false, content_version integer not null default 1,
  revision integer not null default 1, updated_at timestamptz not null default now(),
  check (entry->>'correct_answer' in ('O','X')), check (length(entry->>'prompt') > 0),
  check (entry->>'id' = id), check (entry->>'chapter_id' = chapter_id)
);
create index ox_questions_chapter_status on public.ox_questions(chapter_id,status,id);
create table public.ox_attempts (
  student_id text not null references public.students(id), submission_id uuid not null,
  question_id text not null references public.ox_questions(id), content_version integer not null,
  answer text not null check (answer in ('O','X')), correct boolean not null,
  is_first boolean not null, answered_at timestamptz not null default now(),
  primary key (student_id,submission_id)
);
create unique index ox_attempts_first_unique on public.ox_attempts(student_id,question_id,content_version) where is_first;
create index ox_attempts_first_statistics on public.ox_attempts(question_id,content_version,correct) where is_first;
create index ox_attempts_student_time on public.ox_attempts(student_id,answered_at);
create table public.ox_progress (
  student_id text not null references public.students(id), question_id text not null references public.ox_questions(id),
  content_version integer not null, answer text not null, correct boolean not null,
  wrong_count integer not null default 0, answered_at timestamptz not null default now(),
  primary key(student_id,question_id,content_version)
);
create table public.ox_notes (
  student_id text not null references public.students(id), question_id text not null references public.ox_questions(id),
  memo text not null default '', bookmark boolean not null default false, mastered_version integer,
  updated_at timestamptz not null default now(), primary key(student_id,question_id)
);
create table public.ox_question_history (
  id bigint generated always as identity primary key, question_id text not null references public.ox_questions(id),
  actor text not null, before_value jsonb, after_value jsonb not null, changed_at timestamptz not null default now()
);
create index ox_question_history_question_time on public.ox_question_history(question_id,changed_at desc);

create function public.ox_service(p_action text, p_actor jsonb, p_body jsonb default '{}'::jsonb)
...
begin
  ...
  if not v_admin and not exists(select 1 from public.students where id=v_student and is_active) then raise exception 'unauthorized'; end if;
  ...
  if p_action='status' then return jsonb_build_object('ok',true,'enabled',(select enabled from public.ox_config where id)); end if;
...
alter table public.ox_config enable row level security;
alter table public.ox_collections enable row level security;
alter table public.ox_chapters enable row level security;
alter table public.ox_questions enable row level security;
alter table public.ox_attempts enable row level security;
alter table public.ox_progress enable row level security;
alter table public.ox_notes enable row level security;
alter table public.ox_question_history enable row level security;
revoke all on public.ox_config,public.ox_collections,public.ox_chapters,public.ox_questions,public.ox_attempts,public.ox_progress,public.ox_notes,public.ox_question_history from public,anon,authenticated;
grant select,insert,update,delete on public.ox_config,public.ox_collections,public.ox_chapters,public.ox_questions,public.ox_attempts,public.ox_progress,public.ox_notes,public.ox_question_history to service_role;
grant usage,select on sequence public.ox_question_history_id_seq to service_role;
