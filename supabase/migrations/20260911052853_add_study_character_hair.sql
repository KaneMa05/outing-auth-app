-- Apply before deploying the character selector. Existing profiles retain the default hairstyle.
-- Applied to RONPARK APP on 2026-09-11; filename matches the remote migration version.
begin;
set local lock_timeout = '5s';
alter table public.study_cafe_profiles
  add column if not exists hair_style text not null default 'default'
  constraint study_cafe_profiles_hair_style_check
  check (hair_style in ('default', 'sport', 'spiky', 'mushroom', 'wave', 'ponytail'));
