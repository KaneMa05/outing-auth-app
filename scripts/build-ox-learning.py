...
runtime=runtime.replace("return { openBookmarks() { route='bookmarks'; render(); } };", "return { openBookmarks() { if(pending)return; route='bookmarks'; render(); } };")
...
(root/'criminal-law-ox.css').write_text((root/'docs/criminal-law-ox-proposal/local-app-preview.css').read_text(encoding='utf-8').replace('/* Local OX only.', '/* Scoped OX learning styles.'),encoding='utf-8')
