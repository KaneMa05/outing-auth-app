...
(async()=>{
  ...
  await db.exec(read('supabase/migrations/20260922061015_ox_member_track_filter.sql'));
    ...
    assert.equal(await evaluate("Array.from(document.querySelectorAll('.ox-admin-member-list>li')).every(row=>Array.from(row.querySelectorAll('button')).filter(b=>b.getClientRects().length).length===1)"),true,'Each member shows only one action button');
    await click('[data-admin=member-menu][data-id=offline]');
    assert.equal(await evaluate("document.querySelector('[data-admin=book-add][data-id=offline]').getClientRects().length>0"),true);
    await click('[data-admin=member-menu][data-id=managed]');
    assert.equal(await evaluate("document.querySelectorAll('[data-admin=member-menu][aria-expanded=true]').length"),1);
    assert.equal(await evaluate("document.querySelector('[data-admin=member-menu][data-id=offline]').getAttribute('aria-expanded')"),'false');
    await evaluate("document.querySelector('[data-admin=member-menu][data-id=managed]').dispatchEvent(new KeyboardEvent('keydown',{key:'Escape',bubbles:true}))");
    assert.equal(await evaluate("document.querySelectorAll('[data-admin=member-menu][aria-expanded=true]').length"),0);
    assert.equal(await evaluate("document.activeElement.dataset.id"),'managed');
    ...
    for(const [track,studentId] of [['경찰직 - 공채(순경)','offline'],['수사특채','managed'],['unassigned','lecture']]){
      await evaluate(`document.querySelector('[data-member-track]').value=${JSON.stringify(track)};document.querySelector('[data-member-track]').dispatchEvent(new Event('change'))`);
      await wait(`document.querySelectorAll('[data-admin=member-menu]').length===1&&document.querySelector('[data-admin=member-menu]').dataset.id==='${studentId}'`);
    }
    await evaluate("document.querySelector('[data-member-track]').value='수사특채';document.querySelector('[data-member-track]').dispatchEvent(new Event('change'))");
    await wait("document.querySelector('[data-admin=member-menu]')?.dataset.id==='managed'");
    await evaluate("document.querySelector('[data-member-cohort]').value='18';document.querySelector('[data-member-cohort]').dispatchEvent(new Event('change'))");
    await wait("document.querySelector('[data-admin=member-menu]')?.dataset.id==='offline'");
    assert.equal(await evaluate("document.querySelector('[data-member-track]').value"),'','Changing cohort resets the track');
    assert.deepEqual(await evaluate("Array.from(document.querySelector('[data-member-track]').options).map(o=>o.value)"),['','경찰직 - 공채(순경)','unassigned'],'Only tracks belonging to the cohort are offered');
    await evaluate("document.querySelector('[data-member-track]').value='';document.querySelector('[data-member-cohort]').value='';document.querySelector('[data-member-track]').dispatchEvent(new Event('change'))");
    await wait("document.querySelectorAll('[data-admin=member-menu]').length===3");
    ...
    for(const width of [390,1100]) {
      ...
      await click('[data-admin=member-menu][data-id=lecture]');
      assert.equal(await evaluate("(()=>{const r=document.querySelector('[data-admin=member-menu][data-id=lecture]').nextElementSibling.getBoundingClientRect();return r.width>0&&r.left>=0&&r.right<=innerWidth})()"),true,`Member menu fits at ${width}`);
      await click('[data-admin=member-menu][data-id=lecture]');
    ...
    await db.exec("reset role;insert into students(id,name,class_name,student_category,cohort) select 'pick-'||i,'명단 검증 '||lpad(i::text,2,'0'),'오프라인반','offline',18 from generate_series(1,32)i;insert into lecture_applications(approved_student_id,status,lecture_id) values('offline','approved','Offline_ID');set role service_role;");
    await evaluate("renderCriminalLawOxLocalPreview.view?.controller?.destroy();renderCriminalLawOxLocalPreview.view=null;window.learning=false;window.myPage=false;document.querySelector('#app').hidden=false;document.querySelector('#student').replaceChildren();render()");
    await click('[data-admin=grants]');await wait("document.querySelector('[data-grant-form]')");
    assert.equal(await evaluate("getComputedStyle(document.querySelector('.ox-admin-filters')).display==='none'"),true,'Question management is hidden during grant work');
    await click('[data-grant-close]');await wait("!document.querySelector('.ox-grant-focused')");
    assert.equal(await evaluate("getComputedStyle(document.querySelector('.ox-admin-filters')).display!=='none'"),true,'Returning restores the management screen');
    await db.exec("reset role;update students set track='수사특채' where id in ('pick-2','pick-3');set role service_role;");
    await click('[data-admin=grants]');await wait("document.querySelector('[data-grant-form]')");
    ...
    assert.equal(await evaluate("document.querySelector('[data-grant-review]').disabled"),true,'No student is selected implicitly');
    await click('[data-grant-student=pick-1]');
    await evaluate("document.querySelector('[data-grant-track]').value='수사특채';document.querySelector('[data-grant-track]').dispatchEvent(new Event('change'))");
    assert.equal(await evaluate("document.querySelectorAll('[data-grant-student]').length"),2,'Track narrows the cohort roster');
    assert.equal(await evaluate("document.querySelector('[data-grant-review]').disabled"),true,'Track changes clear the previous selection');
    await click('[data-grant-all]');
    assert.equal(await evaluate("document.querySelector('[data-grant-count]').textContent.includes('2명 선택')"),true,'Select all stays inside the chosen track');
    await evaluate("document.querySelector('[data-grant-search]').value='명단 검증 02';document.querySelector('[data-grant-search]').dispatchEvent(new Event('input'))");
    assert.equal(await evaluate("document.querySelectorAll('[data-grant-student]').length"),1,'Name search intersects track');
    assert.equal(await evaluate("document.querySelector('[data-grant-count]').textContent.includes('2명 선택')"),true,'Search keeps the explicit selection');
    await evaluate("document.querySelector('[data-grant-track]').value='';document.querySelector('[data-grant-track]').dispatchEvent(new Event('change'))");
    assert.equal(await evaluate("document.querySelector('[data-grant-review]').disabled"),true);
    await click('[data-grant-all]');assert.equal(await evaluate("document.querySelector('[data-grant-count]').textContent.includes('33명 선택')"),true,'All selects the full cohort across pages');
    await click('[data-grant-student=pick-1]');await click('[data-roster-next]');await click('[data-roster-prev]');
    assert.equal(await evaluate("document.querySelector('[data-grant-student=pick-1]').checked"),false,'Exclusion survives pagination');
    await click('[data-grant-none]');
    assert.equal(await evaluate("document.querySelector('[data-grant-review]').disabled"),true);
    await evaluate("document.querySelector('[data-grant-search]').value='offline_id';document.querySelector('[data-grant-search]').dispatchEvent(new Event('input'))");
    await wait("document.querySelector('[data-grant-student=offline]')");
    await click('[data-grant-student=offline]');
    assert.equal(await evaluate("document.querySelector('[data-grant-count]').textContent.includes('1명 선택')"),true,'Lecture ID search supports individual selection');
    for(const width of [390,1100]){await send('Emulation.setDeviceMetricsOverride',{width,height:950,deviceScaleFactor:1,mobile:width<500});assert.equal(await evaluate('document.documentElement.scrollWidth<=innerWidth'),true,`Recipient picker overflow at ${width}`);}
    await evaluate("document.querySelector('[data-grant-form] [name=reason]').value='18기 재원생 지원';document.querySelector('[data-grant-form]').requestSubmit()");
