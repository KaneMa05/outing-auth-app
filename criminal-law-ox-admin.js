...
function renderCriminalLawOxAdmin() {
  ...
  if(!document.querySelector('link[data-ox-admin-style]')) document.head.appendChild(el('link',{rel:'stylesheet',href:'./criminal-law-ox-admin.css?v=20260921-ox-book-access','data-ox-admin-style':'true'}));
  ...
  let memberPage=0, memberSearch='', registeredOnly=true, memberRequestId=0, memberTotal=0, memberCollection='', memberStatus='';
  ...
  Object.assign(messages,{access_conflict:'다른 관리자가 이용 권한을 변경했습니다. 목록을 새로고침한 뒤 다시 등록해주세요.',book_already_active:'이미 이용 중인 교재입니다. 목록을 새로고침해주세요.',book_selection_required:'구매한 교재를 선택해 등록해주세요.',invalid_request:'교재·구매일·처리 사유를 확인해주세요.'});
  ...
  function shell() {
    host.innerHTML=`<div class="ox-admin-heading"><div><h2>형사법 OX 관리</h2><p>문항을 검토하고 수강생에게 공개하세요.</p></div><button class="btn" data-admin="new" ${write?'':'disabled'}>문제 추가</button></div>
      ...
      <section class="ox-admin-members" aria-labelledby="ox-members-title"><h3 id="ox-members-title">교재 구매 · 이용 수강생 관리</h3><p>구매한 교재만 선택해 개방하세요. 이용을 중지해도 풀이 기록과 메모는 보존됩니다.</p>
        ...
        <div class="ox-admin-member-filters"><label>교재<select data-member-collection><option value="">전체 교재</option>${books.map(([id,name])=>`<option value="${id}" ${memberCollection===id?'selected':''}>${name}</option>`).join('')}</select></label><label>이용 상태<select data-member-status><option value="">전체 상태</option><option value="active" ${memberStatus==='active'?'selected':''}>이용 중</option><option value="stopped" ${memberStatus==='stopped'?'selected':''}>중지</option></select></label></div>
        <p role="status" data-member-message></p><div data-members></div><div data-book-editor></div>
      </section>
    ...
    for(const selector of ['[data-member-collection]','[data-member-status]']) host.querySelector(selector).onchange=()=>{if(busy)return;memberCollection=host.querySelector('[data-member-collection]').value;memberStatus=host.querySelector('[data-member-status]').value;memberPage=0;loadMembers();};
    list();
    loadMembers();
  }
  async function loadMembers(successMessage='') {
    ...
    try {
      const data=await api('admin_members',{page:memberPage,search:memberSearch,registeredOnly,collectionId:memberCollection,bookStatus:memberStatus});
      ...
      host.querySelector('[data-members]').innerHTML=`<div class="ox-admin-list-head"><span>${memberTotal.toLocaleString()}명</span><span>${memberPage+1} / ${Math.max(1,Math.ceil(memberTotal/30))} 페이지</span></div><ul class="ox-admin-member-list">${data.items.map(s=>`<li><div><strong>${escape(s.name)}</strong><span>${escape(categoryNames[s.student_category] || s.student_category)} · ${escape(s.class_name)}</span><small>학생 ID: ${escape(s.id)}${s.is_active?'':' · 비활성 계정'}</small><div class="ox-admin-book-statuses">${books.map(([id,name])=>{const b=s.books?.find(x=>x.collection_id===id);return `<span class="ox-admin-book-status ${b?.active&&s.allowed?'active':''}">${name} · ${!b?'미등록':b.active&&s.allowed?'이용 중':'중지'}${b?.source==='legacy'?' (기존 이용자)':''}</span>`;}).join('')}</div>${s.books?.length&&!s.allowed?'<small>OX 전체 이용 중지 상태입니다.</small>':''}</div><div class="ox-admin-member-actions"><button class="mini-btn" data-admin="book-add" data-id="${escape(s.id)}" ${write&&s.is_active?'':'disabled'}>교재 구매 등록</button><button class="mini-btn" data-admin="book-stop" data-id="${escape(s.id)}" ${write&&s.books?.some(b=>b.active)?'':'disabled'}>교재 이용 중지</button><button class="mini-btn" data-admin="book-history" data-id="${escape(s.id)}">변경 이력</button>${s.books?.some(b=>b.active)?`<button class="mini-btn" data-admin="member-set" data-id="${escape(s.id)}" data-revision="${s.access_revision}" data-allowed="${!s.allowed}" ${write&&(s.is_active||s.allowed)?'':'disabled'}>${s.allowed?'OX 전체 중지':'OX 전체 재개'}</button>`:''}</div></li>`).join('') || `<li>${registeredOnly?'등록된 수강생이 없습니다. 표시 대상을 전체 수강생으로 바꾸어 등록하세요.':'검색 결과가 없습니다.'}</li>`}</ul><div class="ox-admin-pagination"><button class="mini-btn" data-admin="member-previous" ${memberPage===0?'disabled':''}>이전</button><button class="mini-btn" data-admin="member-next" ${(memberPage+1)*30>=memberTotal?'disabled':''}>다음</button></div>`;
