...
assert.equal(validateApplication(normalizeApplication({ ...validBody, courseType: "online_managed", lectureId: "" })), "invalid_lecture_id");
assert.equal(validateApplication(normalizeApplication({ ...validBody, courseType: "online_managed", lectureId: "ManagedUser01" })), "");
...
assert.match(appSource, /const requiresLectureId = \["online_managed", "lecture"\]\.includes\(courseTypeSelect\.value\)/);
assert.match(appSource, /lectureIdField\.hidden = !requiresLectureId/);
assert.match(appSource, /lectureIdInput\.required = requiresLectureId/);
