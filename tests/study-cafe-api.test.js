...
assert.match(apiSource, /select=student_id,seat_number,status,last_heartbeat_at,updated_at/);
...
assert.match(apiSource, /updated_at=eq\.\$\{encodeURIComponent\(row\.updated_at\)\}/);
assert.match(apiSource, /Prefer: "return=representation"/);
...
assert.match(apiSource, /const isTargetStudent = row\.student_id === STUDY_CAFE_IDLE_PUSH_STUDENT_ID/);
assert.match(apiSource, /const targetIdleActivityAt = isIdle \? idleActivityAt : lastHeartbeatAt/);
assert.match(apiSource, /last_heartbeat_at=eq\.\$\{encodeURIComponent\(row\.last_heartbeat_at\)\}/);
assert.match(apiSource, /if \(!Array\.isArray\(deletedPresence\) \|\| !deletedPresence\.length\) continue;/);
assert.match(apiSource, /await sendStudyCafeIdleReleasePush\(\{\s*studentId: row\.student_id/);
assert.match(apiSource, /await requestSupabase\(\s*"DELETE",\s*`study_cafe_presence\?student_id=eq\.\$\{encodeURIComponent\(row\.student_id\)\}`/);
