...
assert.match(boardSource, /questionBoardState\.loading\s*\? renderQuestionBoardListLoading\(\)/);
assert.match(boardSource, /function renderQuestionBoardListLoading\(\)[\s\S]*?Array\.from\(\{ length: 3 \}/);
assert.match(boardSource, /questionBoardState\.loading \? "불러오는 중"/);
...
assert.match(styleSource, /\.question-post-skeleton-list\s*\{[^}]*display: grid;[^}]*gap: 10px/);
assert.match(styleSource, /\.question-post-skeleton\s*\{[^}]*min-height: 112px;[^}]*border-radius: 16px/);
assert.match(styleSource, /@keyframes question-board-skeleton/);
...
assert.match(read("index.html"), /question-board\.js\?v=20260902-question-board-skeleton-ui/);
...
assert.match(styleSource, /prefers-reduced-motion: reduce[^}]*\.question-post-skeleton > span/);
